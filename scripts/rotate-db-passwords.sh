#!/bin/sh
# Rotate PostgreSQL app/feed/backup role passwords for an existing stack.
...
echo "Rotating passwords for ${APP_DB_USER}, ${FEED_DB_USER} and ${BACKUP_DB_USER} ..."
docker compose exec -T db psql -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  ...
  --set=backup_password="$NEW_BACKUP" <<'SQL'
...
TMP=$(mktemp)
trap 'rm -f "$TMP"' EXIT
awk -v app="$NEW_APP" -v feed="$NEW_FEED" -v backup="$NEW_BACKUP" '
  BEGIN { updated_app=0; updated_feed=0; updated_backup=0 }
...
echo "Updated .env. Restarting web, feed-worker and backup ..."
docker compose up -d --force-recreate web feed-worker backup
